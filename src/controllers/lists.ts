...
interface ListControllerInterface {
  ...
  get?(input: { db: Db; user: ObjectId , listName: string }): Promise<ListWithId>;  
  setName?(input: {db: Db, id: ObjectId, newName: string}): Promise<ListWithId>;

  addItem?(input: { db: Db, id: ObjectId, idListItem: ObjectId }) : Promise<ListWithId>;
  fetchAllItems?(input: { db: Db, id: ObjectId }): Promise<ListItemWithId[]>;
  removeItem?(input: { db: Db, id: ObjectId, idListItem: ObjectId }) : Promise<ListWithId>;
}
